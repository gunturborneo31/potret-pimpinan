...
function onEsc(e) {
  if (e.key === 'Escape' && previewModal.value) previewModal.value = false
}
...
/* ====== FIX: helper aman untuk baca ekstensi & cek previewable ====== */
...
/* ====== FIX: openPreview aman & selalu jalan saat diklik ====== */
function openPreview() {
  const ext = getExt(props.file?.nama_file)
...
<template>
  ...
  >
    ...
    <button
      @click="toggleMenu"
      class="absolute top-2 right-2 p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700"
      v-if="auth?.user && file.folder.user_id === auth.user.id"
    >
    ...
    <div
      v-if="showMenu"
      class="dropdown-menu absolute right-2 top-8 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded shadow-md z-20 w-40"
    >
      <button
    ...
    <!-- Konten file (klik untuk preview) -->
    <div
    ...
    >
      <img
        v-if="['jpg','jpeg','png','gif','webp','bmp'].includes(getExt(file.nama_file))"
      ...
      />
      <img
        v-else-if="getExt(file.nama_file) === 'pdf'"
        src="/img/pdf-icon.png"
        class="w-full h-40 object-contain rounded-md mb-2"
        alt="PDF Thumbnail"
        @click.stop="openPreview"
      />
      <img
    ...
    <div class="mt-2">
      ...
      <button
        v-if="auth?.user && file.folder.user_id === auth.user.id"
        @click="toggleChecked"
        class="mt-2 text-xs px-2 py-1 rounded font-medium border transition"
        :class="file.checked
  ...
  <!-- Modal Preview (responsif + ESC + klik luar) -->
  <div
    v-if="previewModal"
